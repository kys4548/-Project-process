...
$(function(){
	...
	$("#btnViewMyPosts").click(function(){
		location.href = "${path}/photoboard?searchOption=writer_id&keyword=${sessionScope.id}";
	});
	$("#btnViewMyApplys").click(function(){
		location.href = "${path}/photoboard?searchOption=account_id&keyword=${sessionScope.id}";
	});
});
function list(page){
				...
				<div class="w3-section w3-bottombar w3-padding-16">
					...
					<c:choose>
						...
						<c:when test="${sessionScope.role=='performer'}">
								<b>추천 게시글 :</b>
								<c:forEach var="row" items="${map.recommendConcert }">
									<a href="${path}/photo_board/view?id=${row.id}
								&curPage=${map.pager.curPage}
								&searchOption=${map.searchOption}
								&keyword=${map.keyword}" class="anchorNoDeco w3-hover-opacity">[${row.id}] ${row.title}</a>&nbsp;
								</c:forEach>
						</c:when>
					</c:choose>
						...
						<c:choose>
							...
							<c:when test="${sessionScope.role=='performer'}">
								<input type="button" id="btnViewMyApplys" value="내가 지원한 글보기" class="w3-bar-item w3-button w3-black w3-hover-grey">
								<div class="w3-bar-item w3-quarter"></div>
							</c:when>
						</c:choose>
