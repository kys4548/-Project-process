...
public class AccountDAOImpl implements AccountDAO {
	...
	@Override
	public void deleteAll(int accountId, ArrayList<Integer> concerts) {
		...
		for(int i=0; i<concerts.size(); i++) {
			int concertId = concerts.get(i);
			sqlSession.delete("account.delete_concert_instrument",concertId);
			sqlSession.delete("account.delete_concert_tag",concertId);
		}
		sqlSession.delete("account.delete_concert",accountId);
	}
	
	@Override
	public List<AccountDTO> getApplicants(int concert_id){
		return sqlSession.selectList("account.getApplicants",concert_id);
	}
